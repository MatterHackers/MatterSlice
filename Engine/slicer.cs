        ...
        public void makePolygons(OptimizedVolume ov, bool keepNoneClosed, bool extensiveStitching)
        {
            for (int startSegment = 0; startSegment < segmentList.Count; startSegment++)
            {
                ...
                while (true)
                {
                    ...
                    IntPoint p0 = segmentList[segmentIndex].end;
                    poly.Add(p0);
                    int nextIndex = -1;
...
#warning this code needs to be turned back on
            ...
            while (true)
            {
                ...
                if (bestA == bestB)
                {
                    ...
                    openPolygonList.erase(bestA);
                }
                else
                {
                    if (reversed)
                    {
                        for (int n = openPolygonList[bestB].Count - 1; (int)(n) >= 0; n--)
                        {
                    ...
                    if (bestResult.len < long.MaxValue)
                    {
                        if (bestA == bestB)
                        {
                            ...
                            else if (bestResult.AtoB)
                            {
                                ...
                                for (int j = openPolygonList[bestA].Count - 1; j >= 0; j--)
                                {
                                    polygonList[n].Add(openPolygonList[bestA][j]);
                                }
            ...
            if (keepNoneClosed)
            {
                while (openPolygonList.Count > 0)
                {
                    if (openPolygonList[0].Count > 0)
                    {
                        polygonList.Add(openPolygonList[0]);
                    }
            ...
            for (int i = 0; i < polygonList.Count; i++)
            {
                ...
                {
                    length += (polygonList[i][n] - polygonList[i][n - 1]).Length();
        ...
        closePolygonResult findPolygonPointClosestTo(IntPoint input)
        {
            ...
            for (int n = 0; n < polygonList.Count; n++)
            {
                ...
                for (int i = 0; i < polygonList[n].Count; i++)
                {
                    ...
                    if (lineLength > 1)
                    {
                        long distOnLine = IntPoint.Dot(pDiff, input - p0) / lineLength;
                        if (distOnLine >= 0 && distOnLine <= lineLength)
                        {
                            IntPoint q = p0 + pDiff * distOnLine / lineLength;
                            if ((q - input).IsShorterThen(100))
                            {
    ...
    public class Slicer
    {
        ...
        public Slicer(OptimizedVolume ov, int initial, int thickness, bool keepNoneClosed, bool extensiveStitching)
        {
            ...
            Utilities.Output(string.Format("Layer count: {0}\n", layerCount));
            ...
            {
                Point3 p0 = ov.points[ov.faces[i].index[0]].point;
                Point3 p1 = ov.points[ov.faces[i].index[1]].point;
                Point3 p2 = ov.points[ov.faces[i].index[2]].point;
                int minZ = p0.z;
                int maxZ = p0.z;
            ...
            {
                percDone = 100 * layerNr / layers.Count;
                if (t.ElapsedMilliseconds > 2000)
                {
                    Utilities.Output(string.Format("\rProcessing layers... ({0} percent)", percDone));
                }
        ...
        public void dumpSegments(string filename)
        {
