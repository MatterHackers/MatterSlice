...
namespace MatterHackers.MatterSlice
{
	...
	{
		public bool throughAir = false; //!< Whether the path is one which moves through air.
		public bool cross_boundary = false; //!< Whether the path crosses a boundary.

		public void push_back(IntPoint point)
		{
			this.Add(point);
		}
	}
	...
	public class LinePolygonsCrossings
	{
		...
		public struct Crossing
		{
			...
			public long x; //!< x coordinate of crossings between the polygon and the scanline.
			public int point_idx; //!< The index of the first point of the line segment which crosses the scanline
         ...
         */
		public class PolyCrossings
		{
			public int poly_idx; //!< The index of the polygon which crosses the scanline
			public Crossing min; //!< The point where the polygon first crosses the scanline.
			public Crossing max; //!< The point where the polygon last crosses the scanline.
								 /*!
		...
		{
			//int part_idx;
			public void push_back(PolyCrossings extra)
			{
				this.Add(extra);
			}
		...
		void calcScanlineCrossings()
		{
			...
			{
				PolyCrossings minMax = new PolyCrossings(poly_idx);
				Polygon poly = boundary[poly_idx];
		 ...
		 */
		void getCombingPath(CombPath combPath, long max_comb_distance_ignored = 1500)
		{
			if ((endPoint - startPoint).shorterThen(max_comb_distance_ignored) || !lineSegmentCollidesWithBoundary())
			{
			...
			CombPath basicPath = new CombPath();
		 ...
		 */
		void getBasicCombingPath(PolyCrossings polyCrossings, CombPath combPath)
		{
			Polygon poly = boundary[polyCrossings.poly_idx];
			combPath.push_back(transformation_matrix.unapply(new IntPoint(polyCrossings.min.x + dist_to_move_boundary_point_outside, transformed_startPoint.Y)));
			...
			{ // follow the path in the same direction as the winding order of the boundary polygon
				...
				{
					combPath.push_back(PolygonHelper.getBoundaryPointWithOffset(poly, point_idx, dist_to_move_boundary_point_outside));
				}
			}
			else
			...
			combPath.push_back(transformation_matrix.unapply(new IntPoint(polyCrossings.max.x - dist_to_move_boundary_point_outside, transformed_startPoint.Y)));
		}
		...
		{
			PolyCrossings ret = null;
			foreach (PolyCrossings crossing in crossings)
			{
				if (crossing.min.x > x && (ret == null || crossing.min.x < ret.min.x))
				{
					ret = crossing;
				}
			}
		...
		bool optimizePath(CombPath comb_path, CombPath optimized_comb_path)
		{
			...
			for (int point_idx = 1; point_idx < comb_path.size(); point_idx++)
			{
				...
				if (PolygonsHelper.polygonCollidesWithlineSegment(boundary, current_point, comb_path[point_idx]))
				{
					if (PolygonsHelper.polygonCollidesWithlineSegment(boundary, current_point, comb_path[point_idx - 1]))
					{
				...
				else
				{
					...
					{
						if (PolygonsHelper.polygonCollidesWithlineSegment(boundary, optimized_comb_path[optimized_comb_path.size() - 2], comb_path[point_idx]))
						{
							break;
						}
						else
		...
		LinePolygonsCrossings(Polygons boundary, IntPoint start, IntPoint end, long dist_to_move_boundary_point_outside)
		{
			...
			this.endPoint = end;
			this.dist_to_move_boundary_point_outside = dist_to_move_boundary_point_outside;
		}
		 ...
		 */
		static void comb(Polygons boundary, IntPoint startPoint, IntPoint endPoint, CombPath combPath, long dist_to_move_boundary_point_outside, long max_comb_distance_ignored = 1500)
		{
			LinePolygonsCrossings linePolygonsCrossings = new LinePolygonsCrossings(boundary, startPoint, endPoint, dist_to_move_boundary_point_outside);
	...
	public class Comb
	{
		...
		static readonly long max_moveOutside_distance2 = long.MaxValue; //!< Any point which is not inside should be considered outside.
		static readonly long offset_dist_to_get_from_on_the_polygon_to_outside = 40; //!< in order to prevent on-boundary vs crossing boundary confusions (precision thing)
		static readonly long offset_extra_start_end = 100; //!< Distance to move start point and end point toward eachother to extra avoid collision with the boundaries.
		...
		Polygons getLayerSecondWalls()
		{
			...
			{ // when a raft is present
				return storage.raftOutline.Offset(100);
			}
			else
			{
				Polygons layer_walls;
				foreach (SliceMeshStorage mesh in storage.meshes)
				{
					foreach (SliceLayerPart part in mesh.layers[layer_nr].parts)
					{
     ...
     */
	CombComb(SliceDataStorage storage, int layer_nr, long comb_boundary_offset, bool travel_avoid_other_parts, long travel_avoid_distance)
		{
			this.storage = storage;
			this.layer_nr = layer_nr;
			this.offset_from_outlines = comb_boundary_offset; // between second wall and infill / other walls
			this.max_moveInside_distance2 = offset_from_outlines * 2 * offset_from_outlines * 2;
			this.offset_from_outlines_outside = travel_avoid_distance;
			this.avoid_other_parts = travel_avoid_other_parts;
			// , boundary_inside( boundary.offset(-offset_from_outlines) ) // TODO: make inside boundary configurable?
			this.boundary_inside = getLayerSecondWalls();
			this.boundary_outside = null;
			this.partsView_inside = boundary_inside.splitIntoPartsView(); // !! changes the order of boundary_inside !!
		}

		//! Utility function for `boundary_inside.inside(p)`.
		bool inside(IntPoint p)
		{
			return boundary_inside.inside(p);
		}
		 ...
		 */
		bool calc(IntPoint startPoint, IntPoint endPoint, CombPaths combPaths, bool startInside = false, bool endInside = false, long max_comb_distance_ignored = 1500)
		{
			if ((endPoint - startPoint).shorterThen(max_comb_distance_ignored))
			{
				return true;
			}
